...
def ext_wm(img, passcode, channel, channel_invert):
	...
	try:
		...
		wm_imgs = bwm.extract(cv2.cvtColor(img, cv2.COLOR_RGB2BGR), channel="".join(channel), invert="".join(channel_invert))
				...
				with gr.Row():
					...
					with gr.Column(variant="panel"):
						...
						with gr.Row(variant="panel"):
							channel_wm = gr.CheckboxGroup(["Mix","Y","U","V"],value=["Mix","Y","U","V"], label="Channels:", interactive=True)
...
class SDBWM(scripts.Script):
	...
	def ui(self, is_img2img):
		SDBWM.force_last = gr.Checkbox(label='Make SD Blind Watermark run after any other extensions (will reload WebUI)', value=get_force(), interactive=True, elem_id="sdbwm_force_last")
		setattr(SDBWM.force_last,"do_not_save_to_config",True)
		
		SDBWM.force_last.change(
			fn=make_last,
			_js='restart_reload',
			inputs=[],
			outputs=[],
